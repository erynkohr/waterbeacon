from app.models import EpaSystem
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        basedir = settings.BASE_DIR  #not working :/
        jsonDirectory = os.path.join(basedir, '/epaData/stateJson')
        jsonDirectory = '/Users/david/mystuff/codefordenver/waterbeacon/epaData/stateJson'
        ...
        for filename in os.listdir(jsonDirectory):
            ...
            for system in data["Results"]["WaterSystems"]:
                if system['PWSActivityCode'] != 'A':
                    continue # we only want active sites. 
                try:
                    add_system_to_db(system)
                except:
                    print(system)
                    raise
